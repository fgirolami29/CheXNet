...
# 4. Training Loop
# 4. Training Loop con barra di progresso
def train_and_save(model, train_dataset, rng, epochs=EPOCHS, checkpoint_path=CHECKPOINT_PATH):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_dataset), desc=f"EPOCH {epoch + 1}", unit="batch", dynamic_ncols=True) as pbar:
            for i, batch in enumerate(train_dataset):
                ...
                # Aggiorna le informazioni della barra di progresso
                pbar.set_postfix(
                    loss=f"{loss:.4f}",
                    tempo=f"{time.time() - batch_start_time:.4f}s"
                )
                pbar.update(1)
